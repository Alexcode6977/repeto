...
import { saveTroupeScriptWithImportValidation } from "../actions";
...
import { getUserScripts } from "@/lib/actions/play";

interface TroupeImportWizardProps {
    troupeId: string;
...
export function TroupeImportWizard({
    troupeId,
    showImportGuide,
    setShowImportGuide,
    onImportComplete,
    onError,
}: TroupeImportWizardProps) {
    // --- STATE ---
    ...
    // Choice screen state
    const [importChoice, setImportChoice] = useState<"choice" | "catalog" | "library">("choice");

    // L'état de la bibliothèque utilisateur
    const [userScripts, setUserScripts] = useState<any[]>([]);
    const [isLoadingLibrary, setIsLoadingLibrary] = useState(false);

    useEffect(() => {
        if (importChoice === "library") {
            setIsLoadingLibrary(true);
            getUserScripts().then(scripts => {
    ...
    const finalizeImportWithDiagnostics = async () => {
        ...
        try {
            ...
            const saveResult = await saveTroupeScriptWithImportValidation(troupeId, pendingScriptForSave, submission);
    ...
    return (
        <>
                ...
                <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 p-0 md:p-4" onClick={() => { setShowImportGuide(false); }}>
                    <div className="bg-card border border-border rounded-t-3xl md:rounded-3xl p-6 md:p-4 sm:p-6 md:p-8 w-full max-w-4xl shadow-2xl animate-in md:zoom-in-95 slide-in-from-bottom-5 md:slide-in-from-bottom-0 duration-300 relative max-h-[95dvh] md:max-h-[90dvh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="text-center mb-8 md:mb-10 mt-2 md:mt-0">
                            <h2 className="text-2xl md:text-3xl font-extrabold text-foreground tracking-tight">Ajouter une pièce à la troupe</h2>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                            ...
                            {/* 2. Library Import (Personal) */}
                            <button
                                onClick={() => setImportChoice("library")}
                                className="cursor-pointer group relative flex flex-row md:flex-col items-center md:items-stretch h-auto md:h-full text-left bg-card border border-border hover:border-purple-500/50 p-4 md:p-6 rounded-2xl md:rounded-3xl transition-all shadow-sm md:shadow-none hover:shadow-xl w-full active:scale-[0.98]"
                            >
                                <div className="absolute inset-0 bg-purple-500/5 rounded-2xl md:rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity border-2 border-transparent md:border-purple-500/50" />
                                <div className="shrink-0 w-12 h-12 md:w-20 md:h-20 bg-purple-500/10 rounded-xl md:rounded-full flex items-center justify-center mr-4 md:mr-0 md:mb-6 group-hover:bg-purple-500/20 transition-colors z-10">
                                    <FolderSync className="w-6 h-6 md:w-10 md:h-10 text-purple-500" />
                                </div>
                                <div className="flex-1 md:flex md:flex-col md:text-center z-10">
                                    <h3 className="font-bold text-[15px] md:text-xl text-foreground md:mb-3">Ma Bibliothèque</h3>
                                    <p className="text-muted-foreground text-[12px] md:text-sm leading-snug md:leading-relaxed md:flex-grow">
                                        Importez une pièce déjà préparée dans votre espace personnel.
                                    </p>
                                </div>
                                <div className="hidden md:flex w-full mt-6 py-3 rounded-xl bg-purple-500/10 text-purple-600 font-bold group-hover:bg-purple-500 group-hover:text-white transition-colors justify-center z-10">
                                    Parcourir
                                </div>
                                <ChevronRight className="md:hidden w-5 h-5 text-muted-foreground group-hover:text-purple-500 ml-2 shrink-0 z-10" />
                            </button>
            ...
            {/* 6. LIBRARY BROWSER */}
            {showImportGuide && importChoice === "library" && (
                <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 p-0 md:p-4">
                    <div className="bg-card border border-border rounded-t-3xl md:rounded-3xl p-6 md:p-4 sm:p-6 md:p-8 w-full max-w-4xl shadow-2xl relative max-h-[95dvh] flex flex-col h-[600px]">
                        <button onClick={() => setImportChoice("choice")} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground p-2 rounded-full hover:bg-muted"><X className="w-5 h-5" /></button>
                        <h2 className="text-2xl font-bold mb-6">Ma Bibliothèque</h2>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                            {isLoadingLibrary ? (
                                ...
                                <p className="text-center text-muted-foreground py-10">Vous n'avez aucune pièce dans votre bibliothèque Solo.</p>
                            ) : (
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {userScripts.map(script => (
                                        <div key={script.id} onClick={async () => {
                                            if (!script.content) { alert("Ce script semble vide."); return; }
                                            try {
                                                setIsImporting(true);
                                                const title = script.title || "Pièce Importée";
                                                const parsedScript = script.content;
                                                
                                                // Pour le clone depuis la bibliothèque on crée directement la pièce dans la troupe sans repasser par le processus
                                                const { createPlay } = await import("@/lib/actions/play");
                                                await createPlay(troupeId, title, parsedScript);
                                        ...
                                        }} className="p-4 border border-border rounded-xl flex items-center justify-between cursor-pointer hover:border-purple-500/50 hover:bg-purple-500/5 transition-colors">
                                            <div>
                                                <h3 className="font-semibold text-[15px]">{script.title}</h3>
