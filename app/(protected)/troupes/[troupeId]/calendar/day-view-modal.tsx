...
export function DayViewModal({ isOpen, onClose, date, events, userId, onEventClick, onUpdateAttendance }: DayViewModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[180] md:hidden">
            ...
            {/* Modal */}
            <div className="absolute bottom-0 left-0 right-0 bg-card rounded-t-3xl max-h-[80vh] overflow-hidden animate-in slide-in-from-bottom duration-300 pb-[env(safe-area-inset-bottom)]">
                {/* Handle */}
                ...
                <div className="px-6 py-4 overflow-y-auto max-h-[60vh] space-y-3">
                    {events.map((event) => {
                        const myAttendance = event.event_attendance?.find((a) => a.user_id === userId)?.status || 'unknown';
                        const confirmedCount = event.event_attendance?.filter((a) => a.status === 'present').length || 0;
