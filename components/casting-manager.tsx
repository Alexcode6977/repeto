...
export function CastingManager({ characters, troupeMembers, guests }: CastingManagerProps) {
    ...
    return (
        <div className="space-y-4">
            {characters.map((char) => (
                <div key={char.id} className="flex items-center justify-between p-5 rounded-2xl border border-border bg-white/0 hover:bg-card transition-all group overflow-hidden relative">
                    <div className="flex items-center gap-4 relative">
                        ...
                        <div>
                            <p className="font-bold text-foreground text-lg group-hover:text-primary transition-colors">{char.name}</p>
